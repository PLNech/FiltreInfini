...
class MetadataManager {
  ...
  async fetchMetadata(tabId) {
    try {
      // First verify tab exists (avoid errors for synced/invalid tab IDs)
      try {
        await browser.tabs.get(tabId);
      } catch (tabError) {
        // Tab doesn't exist (likely a synced tab with fake ID)
        // Silently skip - these will be fetched via fetchMetadataFromUrl
        return null;
      }
  ...
  /**
   * Trigger ML classification for tabs (optional, non-blocking)
   * Call after metadata prefetch is complete
   * @param {Array<Object>} tabs - Array of tabs to classify
   * @param {Function} onProgress - Progress callback (optional)
   */
  async classifyTabs(tabs, onProgress = null) {
    // Check if ML classifier is available
    if (typeof window === 'undefined' || typeof window.mlClassifier === 'undefined') {
      console.log('[Metadata] ML Classifier not available, skipping classification');
      return;
    }

    console.log(`[Metadata] Starting ML classification for ${tabs.length} tabs`);

    try {
      await window.mlClassifier.classifyAllTabs(tabs, onProgress);
      console.log('[Metadata] ML classification complete');
    } catch (error) {
      console.error('[Metadata] ML classification failed:', error);
    }
  }
