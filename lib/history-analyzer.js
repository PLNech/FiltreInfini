...
class HistoryAnalyzer {
  ...
  categorizeGenericDomain(domain) {
    // Try ontology first (100k+ domains)
    if (typeof domainOntology !== 'undefined' && domainOntology.isReady()) {
    ...
    // Tech
    if (lowerDomain.match(/github|gitlab|bitbucket|stackoverflow|dev\.to|npm|pypi|cargo|crates/)) {
      return 'tech';
    }

    // Gaming
    if (lowerDomain.match(/steam|game|play|xbox|playstation|nintendo/)) {
      return 'gaming';
    }

    // Adult
    if (lowerDomain.match(/porn|xxx|sex|adult|nsfw/)) {
      return 'adult';
    }

    // Shopping
    if (lowerDomain.match(/amazon|ebay|shop|store|cart|buy/)) {
      return 'shopping';
    }

    // Social
    if (lowerDomain.match(/facebook|instagram|linkedin|tiktok|snapchat/)) {
      return 'social';
    }

    // News
    if (lowerDomain.match(/news|times|post|guardian|bbc|cnn|reuters/)) {
      return 'news';
    }

    // Video/Entertainment
    if (lowerDomain.match(/youtube|vimeo|twitch|netflix|hulu|spotify/)) {
      return 'entertainment';
    }

    // Productivity
    if (lowerDomain.match(/notion|trello|asana|slack|zoom/)) {
      return 'productivity';
    }

    // Learning
    if (lowerDomain.match(/course|learn|edu|academy|tutorial/)) {
      return 'learning';
    }

    // Finance
    if (lowerDomain.match(/bank|paypal|venmo|crypto|trading|invest/)) {
      return 'finance';
    }

    // Health
    if (lowerDomain.match(/health|medical|clinic|doctor|pharma/)) {
      return 'health';
    }

    // Sports
    if (lowerDomain.match(/sport|nfl|nba|mlb|soccer|football/)) {
      return 'sports';
    }

    // Travel
    if (lowerDomain.match(/booking|hotel|flight|airbnb|travel/)) {
      return 'travel';
    }
